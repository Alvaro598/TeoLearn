...
export function playNote(note, duration = 0.45) {
  playTone(noteFrequencies[note] || 440, duration, "triangle", 0.14);
}
...
export function playSequence(sequence = [], stepDuration = 0.25) {
  ...
  sequence.forEach((item, index) => {
    ...
    const start =
      ctx.currentTime +
      (isObject ? (item.step - 1) * stepDuration : index * stepDuration);
...
export function playError() {
  ...
  setTimeout(() => playTone(180, 0.22, "sawtooth", 0.08), 130);
}
...
export function startBackgroundMusic() {
  ...
  backgroundNodes = { oscillators, master };
}

export function stopBackgroundMusic() {
  if (!backgroundNodes) return;

  backgroundNodes.oscillators.forEach((oscillator) => oscillator.stop());
